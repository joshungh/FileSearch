...
class App(QMainWindow):
    ...
    def get_filename(self, dirPath, filename):
        checkData = dirPath
        return (checkData + '/' + filename)

    def search_directory(self, keyword):
        results = []
        for each in self.all_drives:
            for root, dir, files in os.walk('C:/b'):
                for f in files:
                    if os.path.splitext(f)[1] == '.txt':
                        #cur_f = open('C:/b/a.txt')
                        with open(root + '/' + f) as cur_f:
                            if keyword in cur_f.read():
                                results.append(os.path.join(root, f))

                    if os.path.splitext(f)[1] == '.xlsx':
                        #wb = xlrd.open_workbook('C:/b/a.xlsx')
                        wb = xlrd.open_workbook(root + '/' + f)
                        sheet = wb.sheet_by_index(0)
                        for row_num in range(sheet.nrows):
                            for col_num in range(sheet.ncols):
                                cell_obj = sheet.row(row_num)[col_num]
                    ...
                    if os.path.splitext(f)[1] == '.pptx':
                        ...
                        prs = Presentation(root + '/' + f)#('C:/b/a.pptx')
                        for slides in prs.slides:
                            for shape in slides.shapes:
                                if shape.has_text_frame:
